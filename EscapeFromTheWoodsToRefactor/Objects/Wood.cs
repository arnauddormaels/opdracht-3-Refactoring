...
{
    public class Wood //<W, M> where W : class where M : class
    {
        private const int drawingFactor = 8;
        private string path;
        private IDatabank db;//<W,M> db;
        private Random r = new Random(1);
        public int woodID { get; set; }
        public List<Tree> trees { get; set; }
        public List<Monkey> monkeys { get; private set; }
        private Map map;
        public Wood(int woodID, List<Tree> trees, Map map, string path, IDatabank db)//IDatabank<W,M> db)
        {
            this.woodID = woodID;
            this.trees = trees;
        ...
        public void WriteWoodToDB()
        {
            ...
            //ConcurrentBag<DBWoodRecord> records = new ConcurrentBag<DBWoodRecord>();
            ConcurrentQueue<DBWoodRecord> records = new ConcurrentQueue<DBWoodRecord>();
            ...
            foreach (Tree t in trees)
            {
                tasks.Add(Task.Run(() =>
                {
                    ...
                    records.Enqueue(new DBWoodRecord(woodID, t.treeID, t.x, t.y));


                }));
            }
            Task.WaitAll(tasks.ToArray());
            db.WriteWoodRecords(records.ToList());
            Console.ForegroundColor = ConsoleColor.Green;
        ...
        public List<Tree> EscapeMonkey(Monkey monkey)
        {
            ...
            do
            {
                ...
                foreach (Tree t in trees)
                {
                    if ((!visited[t.treeID]) && (!t.hasMonkey))
                    {
                       // double d = Math.Sqrt(Math.Pow(t.x - monkey.tree.x, 2) + Math.Pow(t.y - monkey.tree.y, 2));
                        double dx = t.x - monkey.tree.x;
                        double dy = t.y - monkey.tree.y;
                ...
                if (distanceToBorder * distanceToBorder < distanceToMonkey.First().Key)          //aanpassing , *distance border toegev
                {
                    writeRouteToDB(monkey, route);
                    Console.ForegroundColor = ConsoleColor.White;
                    Console.WriteLine($"{woodID}:end {woodID},{monkey.name}");
